...
ToolCenter 第一方金融行情插件。Page 与 Widget 共用简约深色行情终端，可切换折线图 / K 线图、时间范围与自选产品；设置面板用于维护免费数据源、Twelve Data 凭据和自选列表。
...
- 权限：`network.request`
- 数据源：[Twelve Data](https://twelvedata.com/docs) Basic 免费方案与公开 demo、[Kraken Spot REST](https://docs.kraken.com/api-reference/market-data/get-ohlc-data) 公共现货 OHLC、[Binance USDⓈ-M Futures](https://developers.binance.com/en/docs/products/derivatives-trading-usds-futures/Introduction) 公共期货行情
- 自动刷新：数字资产可见时约 60 秒，Twelve Data 产品约 120 秒；隐藏或卸载后停止
...
股票、外汇和商品使用 Twelve Data REST `time_series`；现货数字资产使用 Kraken REST `OHLC`；数字资产期货使用 Binance USDⓈ-M REST `klines`。刷新频率和实际延迟受接口限额、交易所、品种和市场状态影响，因此界面显示的是“最新获取到的数据”，不是交易撮合级实时流。
...
- 小组件可见时，Kraken/Binance 产品约每 60 秒刷新；Twelve Data 产品约每 120 秒刷新，以降低免费日额度耗尽概率。

Twelve Data 的 `demo` 在本插件中仅用于 AAPL 和 EUR/USD。Basic 免费方案需要注册免费 API Key；免费额度与个人使用条款以供应商当前说明为准。商品以及传统交易所期货通常需要更高套餐或额外市场数据授权。当前免费数字资产入口限定为 Kraken 现货（例如 `BTC/USD`）和 Binance USDⓈ-M 期货（例如 `BTCUSDT`）。

默认自选产品为 AAPL、MSFT、EUR/USD、BTC/USD、BTCUSDT、ETHUSDT 和 SOLUSDT。其中 MSFT 等非 demo 的 Twelve Data 产品需要用户自己的免费 API Key；Kraken 现货与三个 Binance USDⓈ-M 产品不需要 Key。
...
Twelve Data 公开 demo、Kraken 现货和 Binance 期货请求不携带凭据。所有网络请求仍由共享 Rust HTTPS 服务执行并再次校验 `network.request`。宿主只允许 HTTPS GET、标准 443 端口、公开域名、有限响应大小和短超时，并禁止重定向、localhost 与 IP 字面量目标。
...
2. 不填写 API Key，分别验证 AAPL、BTC/USD 和 BTCUSDT 的公开连接；
