...
ToolCenter 第一方金融行情插件。Page 使用完整的简约深色行情终端；桌面 Widget 使用无外框的纯行情视图，完整铺满组件尺寸。支持折线图 / K 线图、时间范围与自选产品；设置面板用于维护免费数据源、可选 Twelve Data 凭据和自选列表。
...
- 桌面 Widget 去除宿主边框、独立标题栏和终端页眉 / 页脚，只保留铺满尺寸的核心行情区；
- 折线图 / K 线图即时切换；
- 1 日 / 5 日 / 1 月范围切换；
