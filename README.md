...
- 受限定制版 FlClash 控制服务，只连接固定的 `127.0.0.1:19090`，读取代理组、切换已验证节点，并通过专用全局快捷键控制 FlClash 主代理开关；
- 内置 HDR 开关 Widget，可选择目标显示器并通过受权限保护的宿主能力切换 HDR；
- 内置 FlClash 控制 Widget，可显示代理运行状态、启动或停止 FlClash 主代理、选择代理组和节点，并在节点选项末尾展示最近测速结果；
...
项目只维护当前工作区这一套正式源码，固定发布目录为 `发布版/`。执行正式构建后会直接覆盖：
...
不再创建 `ToolCenter-v1.0.1/` 一类代码或程序副本。需要保留历史版本时使用 Git 提交和标签回退，不复制项目文件夹。原 `正式版/` 已于 2026-08-11 完成迁移并清理，后续不得重新创建。
...
命令会运行完整前端与 Rust 门禁、构建桌面程序，并覆盖 `发布版/ToolCenter.exe`。正式程序只包含随当前源码构建并通过校验的第一方插件，不会动态执行项目外的未知源码。
